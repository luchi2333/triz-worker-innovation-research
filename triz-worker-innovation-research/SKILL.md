...
| G5 报告与独立复核 | [delivery-contract.md](references/delivery-contract.md)、[engineering-figure-planning.md](references/engineering-figure-planning.md)、[engineering-consistency-review.md](references/engineering-consistency-review.md)、[final-report-blueprint.md](references/final-report-blueprint.md)、[weak-model-playbook.md](references/weak-model-playbook.md) | Report Plan、领域正确的原理/时序/安全图、决策摘要、主报告、证据附件、台账、研究记录与交付清单 | 结构/计算/工程三类状态分列；图文、数值、引用、图序及逐页审查通过 |

进入、完成或阻塞一个阶段时向用户播报一行；暂停时保存当前阶段、已确认内容、待办和恢复入口；恢复时从台账继续，不重做已确认且仍有效的阶段。任何人（含现场人员）否决某个方案或假设时，把否决原文记为 `F`/约束并做影响分析，不静默丢弃。

## 39×39 矛盾矩阵强制查询

技术矛盾必须先按 [engineering-consistency-review.md](references/engineering-consistency-review.md) 写“控制变量 X→改变方向→有用结果→同次改变引发的恶化→因果依据→EC2”，并用两个简化输入核查方向。因果尚未成立时标 `assumption_only/reframe_required/no_triz_contradiction`，不得为了使用 TRIZ 强行进入矩阵。成立后再写现场改善量、恶化量、测量方法、参数候选与选择理由。固定查询方向为“欲改善参数作行，随之恶化参数作列”。
...
先比较不同作用拓扑，再讨论尺寸和零部件。每个候选按 [output-templates.md](references/output-templates.md) 建立完整追溯，并把模块、步骤、范围、接口、主动作用、相互影响和失败回退写入研究记录。测量/识别路线必须完成最小可辨识性卡。成熟来源只能证明来源对象，不能把成熟部件等级迁移为候选系统等级。任何人身安全、对象不可逆损伤、法规、污染/碎屑、失效不可回退或核心专利碰撞硬门槛失败，不能被效率高分抵消。评分锚点统一冻结；未知项写 `?`，不得逐行降低证据门槛。
...
`standard` 和 `engineering` 交付默认主动生成图示与正式 DOCX；用户不需要再次提出“画图”或“做成 Word”。先更新研究记录，再在 `deliverables-manifest.json` 写 Figure Plan。图示须按机械、电气/测量、控制/软件、热/流体或工艺/作业选择正确表达；没有相对运动时不得伪造机械动作序列，电气/测量图不得用算法框图代替真实端口、参考点和回路。图示按实际嵌入尺寸检查有效字号，视觉复核必须绑定被检查文件散列并留下具体发现。平台原生文档工具可用时优先使用；没有文档库但可运行 Python 时，可用 `scripts/build_report.py` 与 `assets/report-source-template.json` 生成跨平台 DOCX。最终填写 schema 1.2 交付清单并运行成果校验器；旧 schema 可读取，但不能被认定为新版完整通过。
...
8. G5 固定执行“研究记录更新→能力探测→Report Plan→领域 Figure Plan→具体发现复核→四类成果→DOCX→manifest→渲染→结构/计算/工程三类状态→交付回执”，不得把图示或 Word 留作后续扩展。
...
- 变量方向、真实矛盾、端到端能力、可辨识性与工程语义复核：[engineering-consistency-review.md](references/engineering-consistency-review.md)
...
- 结构化研究记录模板：[research-record-template.json](assets/research-record-template.json)
