...
`standard` 和 `engineering` 项目必须包含四类成果；四类内容可分别成文件，也可在用户明确只要一种文件时合并，但角色必须在交付清单中逐项映射：

1. `01-决策摘要`：2～3 页或等量篇幅，技术方案导向；
...
Figure Plan 未冻结，或核心推荐方案未达到 [engineering-figure-planning.md](engineering-figure-planning.md) 的最低结构/运动/安全图契约时，不能进入正式排版。
...
第一页优先采用“一句话结论 + 现场瓶颈 + 推荐方案总图/核心动作图 + 当前成熟度 + 下一项试验”。第二页再放路线决策和 TRIZ/查新依据；第三页放验证、效益情景和实施路径。不要让读者到正文后半部才第一次看见核心方案。
...
正文只展开与最终方案直接相关的 IFR/资源、EC1/EC2、主次参数映射、关键矩阵单元、物理矛盾/物—场和 TRIZ—方案映射；完整 40 原理筛选和逐单元查询记录放技术证据附件。详细输出仍按 [triz-analysis-output.md](triz-analysis-output.md) 保存，不因正文压缩而删除。
...
| 成熟已有技术 | 可直接集成的产品、工艺、机构或标准做法；没有或未知时明确写出 |
| 成熟技术来源 | identified 时给原厂手册、专利、论文、标准等来源 ID；其他状态说明未列出原因 |
| 本项目场景化集成 | 如何适配现场长度、空间、人员、工序和接口 |
| 候选创新部分 | 与既有技术相比的差异作用链，不是需求口号；没有新增创新时明确写无 |
| 创新边界 | 明确哪些成熟模块、通用原理、已有做法不属于本项目创新 |
| 解决现有问题 | 逐项说明如何消除/降低原失效 |
| 进一步提升 | 对质量、安全、稳定性、标准化、维护等额外增益 |
| 风险/否证 | 最敏感参数、危险失效、最低成本推翻条件 |
| 验证状态 | F/M/S/H；不得把概念写成已实现 |

“成熟已有技术 / 成熟技术来源 / 本项目场景化集成 / 候选创新部分 / 创新边界”必须在主报告中直接可见，不能只藏在附件或研究记录。成熟模块被移植、组合或参数适配时，先归入场景化集成；只有相对已检得现有技术新增或改变的作用链、接口或协同关系才进入候选创新。没有新增创新时如实写明，不强行包装。

每个进入决策的方案至少配一张自绘原理图。核心推荐方案按 [engineering-figure-planning.md](engineering-figure-planning.md) 至少完成 F4 机理剖面和 F5 运动序列；有安全风险时另画 F7，有连续现场动作时另画 F8。F3 系统架构图只能解释模块关系，不能代替上述技术原理图。

本章先满足 [technical-report-quality.md](technical-report-quality.md) 的六项具体度要求，再安排图文。每张图配有结构/变量对应说明、作用原理和成立条件，按需要增加公式、带中间量的例算与反例，不设固定字数或图文占比。图后给一句 `main_message`，但一句要点不能替代原理解释。表格统一采用白底黑字三线表，流程图及原理图默认黑白技术出版风格。
...
- 引用紧邻命题，来源台账可回链；
- 图表有标题、编号、单位、来源和边界；
- 图像有替代文本；Word 中关键外部来源为可点击链接；表格设置重复表头；
- 不使用“颠覆性、革命性、彻底、绝对、零风险”等无证据修辞；
- 同一术语、模块编号、路线编号和版本在四件成果中一致。

修订必须改写原段落、原表格和原结论；不得保留已被推翻的旧结论，再靠末尾“勘误/补充说明”抵消。交付前对四类成果做全局冲突扫描。
