...
本章先满足 [technical-report-quality.md](technical-report-quality.md) 的五项具体度要求，再安排图文。每张图配有结构/变量对应说明、作用原理和成立条件，按需要增加公式、带中间量的例算与反例，不设固定字数或图文占比。图后给一句 `main_message`，但一句要点不能替代原理解释。表格统一采用白底黑字三线表，流程图及原理图默认黑白技术出版风格。
